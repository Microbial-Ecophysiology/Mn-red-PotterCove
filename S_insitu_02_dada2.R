...
# samples sequenced in multiple libraries and lanes are analysed here together using a loop, merged for ASV creation after error correction
...
path <- paste0("/storage/hdd5/lwu/Amplicon/PotterCove_insitu/seq_by_flowcell/", flowcell_ID, "/Clipped_", TARGET)
...
range_truncLen_A <- matrix(
  c(120, 170, # first number for R1 read (= forward), second number for R2 read (=reverse)
    115, 175,
    110, 180),
...
### seq lane H7W7NDRX2_L2, resequenced library
  ## forward (R1)
  # bad peak around 100 bp, but just ignore that one; bad peak at ~145 bp; some sample drop off already around 120-140 bp;
  # in rf another bad peak at ~105-110 bp
  # potential cut-off: 100-120 or 145 bp
  ## reverse (R2)
  # looks much better than fwd, fr reads only drop after 200 bp; 
  # rf bad deep peak at 180 bp, slight bad peak at ~145 bp; bad peak around 100 bp which has to be ignored
  # potential cut-off: 145 bp or 175-180 bp
sample.paths[['H7W7NDRX2_L2']][['range_truncLen']] <- range_truncLen_A
sample.paths[['H7W7NDRX2_L2']][['range_maxEE']] <- range_maxEE

### seq lane H7WKNDRX2_L2, resequenced library
  ## forward (R1)
  # slight bad peak at ~145 bp; bad peak at fr ~180, rf ~175 bp
  # in rf another bad peak at ~105-110 bp
  # potential cut-off: 175 bp or 100-145 bp
  ## reverse (R2)
  # looks much better than fwd, fr reads only drop after 200 bp; 
  # rf bad peak at 175-180 bp, slight bad peak at ~145 bp;
  # potential cut-off: 145 bp or 175-180 bp or even up to 200 bp, peak is not that bad
sample.paths[['H7WKNDRX2_L2']][['range_truncLen']] <- matrix(
  c(110, 180, # first number for R1 read (= forward), second number for R2 read (=reverse)
    115, 175,
    105, 185,
    100, 190),
...
### seq lane HHMW5DRXX_L1, old library
  ## forward (R1)
  # looks really by in general, also at beginning of read, many samples with few reads;
  # potential cut-off: 120 bp, as short as possible
  ## reverse (R2)
  # looks much better than fwd; fr only drops past 200 bp; rf has some drops at 180 bp
  # potential cut-off: 175-180 bp or 200 bp
sample.paths[['HHMW5DRXX_L1']][['range_truncLen']] <- range_truncLen_B
sample.paths[['HHMW5DRXX_L1']][['range_maxEE']] <- range_maxEE

### seq lane HJ72JDRXX_L2, old library
  ## forward (R1)
  # looks really by in general, also at beginning of read, many samples with few reads;
  # potential cut-off: 90-120 bp, as short as possible
  ## reverse (R2)
  # looks much better than fwd; fr drops past 200 bp; rf has some drops at 100, 145, 180 bp
  # potential cut-off: 145, 175-180 bp, 200 bp
sample.paths[['HJ72JDRXX_L2']][['range_truncLen']] <- range_truncLen_B
sample.paths[['HJ72JDRXX_L2']][['range_maxEE']] <- range_maxEE
...
# seq lane H7W7NDRX2_L2 ##
# truncLen = c(120,170), maxEE = c(2,2);
# index 3, fr: prop.total 0.9468; q10 0.9406; q90 0.9530; rf: prop.total 0.9326; q10 0.9263; q90 0.9388;
sample.paths[['H7W7NDRX2_L2']][['truncLen_R1']] <- 120
sample.paths[['H7W7NDRX2_L2']][['truncLen_R2']] <- 170
sample.paths[['H7W7NDRX2_L2']][['error_R1']] <- 2
sample.paths[['H7W7NDRX2_L2']][['error_R2']] <- 2

# seq lane H7WKNDRX2_L2 ##
# truncLen = c(110,180), maxEE = c(2,2);
# index 3, fr: prop.total 0.9701; q10 0.9653; q90 0.9769; rf: prop.total 0.9610; q10 0.9562; q90 0.9674;
sample.paths[['H7WKNDRX2_L2']][['truncLen_R1']] <- 110
sample.paths[['H7WKNDRX2_L2']][['truncLen_R2']] <- 180
sample.paths[['H7WKNDRX2_L2']][['error_R1']] <- 2
sample.paths[['H7WKNDRX2_L2']][['error_R2']] <- 2

# seq lane HHMW5DRXX_L1 ##
# truncLen = c(90,200), maxEE = c(2,2);
# index 23, fr: prop.total 0.9563; q10 0.9412; q90 0.9712; rf: prop.total 0.9441; q10 0.9270; q90 0.9631;
sample.paths[['HHMW5DRXX_L1']][['truncLen_R1']] <- 90
sample.paths[['HHMW5DRXX_L1']][['truncLen_R2']] <- 200
sample.paths[['HHMW5DRXX_L1']][['error_R1']] <- 2
sample.paths[['HHMW5DRXX_L1']][['error_R2']] <- 2

# seq lane HJ72JDRXX_L2 ##
# truncLen = c(90,200), maxEE = c(2,2);
# index 23, fr: prop.total 0.9481; q10 0.9386; q90 0.9600; rf: prop.total 0.9344; q10 0.9211; q90 0.9481;
sample.paths[['HJ72JDRXX_L2']][['truncLen_R1']] <- 90
sample.paths[['HJ72JDRXX_L2']][['truncLen_R2']] <- 200
sample.paths[['HJ72JDRXX_L2']][['error_R1']] <- 2
sample.paths[['HJ72JDRXX_L2']][['error_R2']] <- 2
...
# samples not reaching convergence:
 # H7WKNDRX2_L2 FR_R1, RF_R1
sink("Logfiles/H7WKNDRX2_L2_Bac_log_errorEstimation.log", append = T)
sample.paths[['H7WKNDRX2_L2']][['errFR_R1']] <- learnErrors(sample.paths[['H7WKNDRX2_L2']][['filtFR_R1']], errorEstimationFunction = loessErrfun2, multithread = threads, randomize = TRUE, verbose = 1, MAX_CONSIST = 20)
sample.paths[['H7WKNDRX2_L2']][['errRF_R1']] <- learnErrors(sample.paths[['H7WKNDRX2_L2']][['filtRF_R1']], errorEstimationFunction = loessErrfun2, multithread = threads, randomize = TRUE, verbose = 1, MAX_CONSIST = 20)
sink()

 # H7WKNDRX2_L2 RF_R1 still not reached convergence
sink("Logfiles/H7WKNDRX2_L2_Bac_log_errorEstimation.log", append = T)
sample.paths[['H7WKNDRX2_L2']][['errRF_R1']] <- learnErrors(sample.paths[['H7WKNDRX2_L2']][['filtRF_R1']], errorEstimationFunction = loessErrfun2, multithread = threads, randomize = TRUE, verbose = 1, MAX_CONSIST = 20)
sink()
...
seqtab.nochim2 <- seqtab.nochim[, colSums(seqtab.nochim) > 1 & ((nchar(colnames(seqtab.nochim)) >= 249 & nchar(colnames(seqtab.nochim)) <= 254) | nchar(colnames(seqtab.nochim)) == 276)]
dim(seqtab.nochim2) 
